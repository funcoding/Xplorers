...
<?php
            if ($current_login== $member) {
                echo ('<div id="media' . $idtable . '"class="media"><img  class="pull-left" style="width: 50px; height: 50px;" alt="http://xplorers-appsbyvinay.rhcloud.com/profilepics/' . $member . '.gif" src="http://xplorers-appsbyvinay.rhcloud.com/profilepics/' . $member . '.jpg"/>' . '<div class="media-body"><h4 class="media-heading"> ' . $post_by . '<input   type="button" name="thuuku" class="btn" onClick="javascript:deletepost(' . $idtable . ')" style="float:right" id="deletepost_' . $idtable . '" value="Delete"/>' . '
</h4>' . '<p>' . htmlentities($posts, ENT_QUOTES) . '<p style="font-size:12px;color:#3B5998;">' . $dtime . '</p>' . '</p></p></div></div>');
            } else {
                echo ('<div id="media' . $idtable . '"class="media"><img  class="pull-left" style="width: 50px; height: 50px;" alt="http://xplorers-appsbyvinay.rhcloud.com/profilepics/' . $member . '.gif" src="http://xplorers-appsbyvinay.rhcloud.com/profilepics/' . $member . '.jpg"/>' . '<div class="media-body"><h4 class="media-heading"> ' . $post_by. '
</h4>' . '<p>' . htmlentities($posts, ENT_QUOTES) . '<p style="font-size:12px;color:#3B5998;">' . $dtime . '</p>' . '</p></div></div>');
            }
            echo ($conn->error);
...
{echo($conn->error);
                ...
                while ($querry2->fetch()) {
                    ...
                    if ($member_comment_id == $current_login) {
                        echo ('<li id="subcomment_no_' . $id1 . 
                        '" class="media" style=" margin-bottom: 1px;background-color:#EDEFF4;margin-left: 50px;margin-top: 0px;">
                        ...
                        alt="http://xplorers-appsbyvinay.rhcloud.com/profilepics/'.$member_comment_id.'.gif" 
                        src="http://xplorers-appsbyvinay.rhcloud.com/profilepics/'.$member_comment_id .'.jpg"/>' . 
                        ...
                        alt="http://xplorers-appsbyvinay.rhcloud.com/profilepics/'.$member_comment_id.'.gif" 
                        src="http://xplorers-appsbyvinay.rhcloud.com/profilepics/'.$member_comment_id.'.jpg"/>' . 
                        '<div class="media-body"><h6 class="media-heading"> </h6><p style="font-size:12px;word-wrap:break-word;padding-right: 10px;">' . '<strong>' . htmlspecialchars($name_of_member) . '</strong>' . " " . htmlentities($display_comments) . '</p><p style="font-size:12px;color:#3B5998;">' . $dtime2 . '</p></div></li>');
                    }
                }
?>
